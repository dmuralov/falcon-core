...
class WalletFalconTest(FalconTestFramework):
    ...
    def falcon_wallet_process(self, *args):
    ...
    def assert_tool_output(self, output, *args):
        p = self.falcon_wallet_process(*args)
    ...
    def run_test(self):
        ...
        p = self.falcon_wallet_process('-h', 'generatemnemonic')
        stdout, stderr = p.communicate()
        assert_equal(p.poll(), 0)
        assert_equal(stderr, '')
        assert('1. language' in stdout)
        p = self.falcon_wallet_process('generatemnemonic')
        stdout, stderr = p.communicate()
        assert_equal(p.poll(), 0)
        assert_equal(stderr, '')
        assert(len(stdout.split(' ')) == 24)

        p = self.falcon_wallet_process('generatemnemonic', 'spanish', '16')
        stdout, stderr = p.communicate()
        assert_equal(p.poll(), 0)
        assert_equal(stderr, '')
