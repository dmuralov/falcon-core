...
ALL_SCRIPTS = EXTENDED_SCRIPTS + BASE_SCRIPTS + FALCON_SCRIPTS + INSIGHT_SCRIPTS + FALCON_SCRIPTS_EXT
...
def main():
    ...
    #    test_list += ALL_SCRIPTS
        ...
        if args.falcon:
            test_list += FALCON_SCRIPTS
        if args.insight:
            test_list += INSIGHT_SCRIPTS
        if args.bitcoin:
            test_list += BASE_SCRIPTS
        if args.falconext:
            test_list += FALCON_SCRIPTS_EXT
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control, create_cache=True):
    ...
    try:
        ...
        if subprocess.run(["pgrep", "-x", "falcond"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a falcond process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
